...
class HandoffService:
    ...
    async def process_question(self, question: str) -> Dict[str, Any]:
        ...
        """
        self.logger.info(
            "Handoff request received", question=question, question_length=len(question)
        )
        ...
        try:
            ...
            try:
                ...
                orchestration_result = await asyncio.wait_for(
                    orchestration.invoke(task=question, runtime=runtime), timeout=30.0
                )

                self.logger.info("Orchestration completed, getting result")

                # Get the final value with timeout
                final_value = await asyncio.wait_for(orchestration_result.get(), timeout=5.0)
                ...
                if tracker_answer:
                    ...
                    return {"agent": agent_name, "answer": tracker_answer.strip()}
                else:
                    return {
                        "agent": "SearchAgent",
            ...
            if tracker_answer and tracker_answer.strip():
                ...
                self.logger.info(
                    "Using tracker response (first valid response)", answer_length=len(answer)
                )
            else:
                # Fallback to extracting from final_value if tracker is empty
                answer = self._extract_answer(final_value)
                self.logger.info(
                    "Using final_value as fallback", answer_length=len(answer) if answer else 0
                )
            ...
            return {"agent": agent_name, "answer": answer.strip()}
    ...
    def _extract_from_message(self, message: Any) -> str:
        ...
        if not answer:
            ...
            if "OrchestrationResult" not in type_name and "Task" not in type_name:
                ...
                self.logger.warning(
                    "Skipping string conversion of task/result object", type_name=type_name
                )
