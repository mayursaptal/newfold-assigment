    ...
    async def stream_chat(self, question: str):
        ...
        try:
            ...
            chat_function = get_plugin_function(
                self.kernel, plugin_name="chat", function_name="stream_chat"
            )
            ...
            async for chunk in self.kernel.invoke_stream(
                function=chat_function, arguments=arguments
            ):
...
Release Year: {film.release_year or 'N/A'}"""
        ...
        try:
            ...
            summarize_function = get_plugin_function(
                self.kernel, plugin_name="film_summary", function_name="summarize_tool"
            )
            ...
            response = await self.kernel.invoke(function=summarize_function, arguments=arguments)
            ...
            rating_value = (
                film.rating.value if film.rating and hasattr(film.rating, "value") else "N/A"
            )
            return {"title": film.title, "rating": rating_value, "recommended": False}
