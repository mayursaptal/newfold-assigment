...
from semantic_kernel.agents import (
    HandoffOrchestration as SKHandoffOrchestration,
...
def create_handoff_orchestration(
    session: AsyncSession, kernel: Kernel
) -> tuple[SKHandoffOrchestration, dict]:
    ...
    async def agent_response_callback(
        message: ChatMessageContent | list[ChatMessageContent],
    ) -> None:
        ...
        if isinstance(message, list):
            if not message:
                return
            # Use the first message for agent tracking
            first_message = message[0]
        else:
            first_message = message

        agent_name = first_message.name or "Unknown"
        ...
        if hasattr(first_message, "content") and first_message.content:
            content = str(first_message.content).strip()

        # Try text attribute
        if not content and hasattr(first_message, "text") and first_message.text:
            content = str(first_message.text).strip()

        # Try items attribute (ChatMessageContent may have items)
        if not content:
            items = None
            if hasattr(first_message, "items"):
                items = first_message.items
            elif hasattr(first_message, "__getitem__") and hasattr(first_message, "__contains__"):
        ...
        role_is_assistant = hasattr(message, "role") and (
            hasattr(message.role, "value")
            and message.role.value == "assistant"
            or str(message.role) == "assistant"
        )
        ...
        if (
            ...
            and content
            and content.strip()
            and not content.startswith("Task is completed")
            and not content.startswith("__CONVERSATION_COMPLETE__")
        ...
        message_attrs = [attr for attr in dir(first_message) if not attr.startswith("_")]
        logger.info(
            "Agent response",
            agent_name=agent_name,
            role=(
                first_message.role.value
                if hasattr(first_message.role, "value")
                else str(first_message.role)
            ),
            content_length=len(content) if content else 0,
            content_preview=content[:500] if content else "",
            has_content=bool(content),
            message_has_content=bool(hasattr(first_message, "content") and first_message.content),
            message_has_text=bool(hasattr(first_message, "text") and first_message.text),
            message_has_items=bool(hasattr(first_message, "items")),
        ...
        if not content and hasattr(first_message, "__dict__"):
            logger.warning(
                "No content extracted, checking message __dict__",
                message_dict_keys=(
                    list(first_message.__dict__.keys())[:10]
                    if hasattr(first_message, "__dict__")
                    else []
                ),
            )
    ...
    def human_response_function() -> ChatMessageContent:
        ...
        """
        call_count: int = agent_tracker.get("human_response_calls", 0)  # type: ignore[assignment]
        agent_tracker["human_response_calls"] = call_count + 1

        logger.debug("Human response function called", call_count=call_count + 1)
